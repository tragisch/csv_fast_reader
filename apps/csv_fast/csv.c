...
#define CSV_STREAM_BUF_SIZE (256U * 1024U)
...
typedef enum { CSV_SOURCE_MMAP, CSV_SOURCE_STREAM } CsvSourceKind;
                                       ...
                                       const CsvOptions *options)
{
    ...
    *out_reader = NULL;
    ...
    /* Reject characters that would break or ambiguate the parser's
     * assumptions. Quote and escape must stay distinct so quoted state and
     * escaped bytes can be recognized unambiguously.
     */
    if (options->delim == options->quote ||
        options->quote == options->escape ||
...
static void CsvCloseMmapResources(CsvReader *handle)
{
    UnmapMem(handle);
    close(handle->fh);
}
...
void csv_reader_close(CsvReader *handle)
{
    ...
    if (handle->sourceKind == CSV_SOURCE_MMAP)
        CsvCloseMmapResources(handle);
    else
        free(handle->streamBuf);
...
CsvStatus csv_reader_open_stream(CsvReader **out_reader, FILE *stream)
{
    ...
    return csv_reader_open_stream_with_options(out_reader, stream, &options);
}

CsvStatus csv_reader_open_stream_with_options(CsvReader **out_reader,
                                              FILE *stream,
                                              const CsvOptions *options)
{
    CsvReader *handle;

    if (!out_reader || !stream || !options)
        return CSV_STATUS_INVALID_ARGUMENT;
    ...
    *out_reader = NULL;
    ...
    handle->delim = options->delim;
    handle->quote = options->quote;
    handle->escape = options->escape;
    handle->lastStatus = CSV_STATUS_OK;
    handle->sourceKind = CSV_SOURCE_STREAM;
    handle->stream = stream;
    handle->blockSize = CSV_STREAM_BUF_SIZE;
    CsvInitCharClass(handle);

    /* Set file handle to invalid so close never accidentally
       closes a real descriptor. */
#if defined(CSV_FAST_PLATFORM_POSIX)
    handle->fh = -1;
#elif defined(_WIN32)
    ...
    handle->streamBuf = malloc(CSV_STREAM_BUF_SIZE);
    if (!handle->streamBuf)
    {
        free(handle);
        return CSV_STATUS_NO_MEMORY;
...
static int CsvEnsureStreamLoaded(CsvReader *handle)
{
    size_t nread;
    ...
    nread = fread(handle->streamBuf, 1, handle->blockSize, handle->stream);
    if (nread == 0)
    {
        handle->streamEof = 1;
        if (ferror(handle->stream))
        {
            handle->lastStatus = CSV_STATUS_IO_ERROR;
            return -1;
        }
        return -EINVAL;
    }

    handle->mem = handle->streamBuf;
    handle->pos = 0;
    handle->size = nread;

    if (nread < handle->blockSize)
        handle->streamEof = 1;

    return 0;
}

static int CsvEnsureInput(CsvReader *handle)
{
    if (handle->sourceKind == CSV_SOURCE_STREAM)
        return CsvEnsureStreamLoaded(handle);
...
{
    /* Newline search is structured as layered fast paths:
     * 1. optional SIMD block filtering on supported targets,
     * 2. word-at-a-time filtering,
     * 3. scalar fallback that updates quote/escape state precisely.
     */
    char* end = p + size;
...
CsvStatus csv_reader_next_row(CsvReader *handle, CsvStringView *out_row)
{
    ...
    do
    {
        ...
        else if (err)
        {
            ...
            /* else: CsvEnsureStreamLoaded already set lastStatus */
            break;
        }
                              ...
                              CsvStringView *out_col)
{
    ...
    if (quoted) {
        char* quotedBegin = p + 1;
        char* quotedEnd = quotedBegin;

        while (*quotedEnd && *quotedEnd != handle->quote && *quotedEnd != handle->escape)
            quotedEnd++;

        if (*quotedEnd == handle->quote && quotedEnd[1] != handle->quote)
        {
            if (quotedEnd[1] == handle->delim || quotedEnd[1] == '\0')
            {
                *quotedEnd = '\0';
                handle->context = quotedEnd + 1;
                if (*handle->context == handle->delim)
                {
                    handle->context++;
                ...
                out_col->ptr = quotedBegin;
                out_col->len = (size_t)(quotedEnd - quotedBegin);
                return CsvSetStatus(handle, CSV_STATUS_OK);
            }
...
CsvStatus csv_reader_first_col(CsvReader *reader,
                               ...
                               CsvStringView *out_col)
{
    return csv_reader_next_col(reader, row, out_col);
}

CsvStatus csv_reader_next_col_in_row(CsvReader *reader, CsvStringView *out_col)
{
    return csv_reader_next_col(reader, NULL, out_col);
}
...
#define CSV_WRITER_BUF_SIZE (256U * 1024U) /* 256 KB write buffer */
...
struct CsvWriter_
{
    ...
    size_t bufPos;
    size_t bufCap;
    size_t colIndex;  /* columns written in the current row */
    char delim;
    char quote;
};

static CsvStatus CsvWriterValidateOptions(const CsvOptions *options)
{
    if (options->delim == options->quote ||
        options->quote == options->escape ||
        options->delim == '\n' || options->delim == '\r' || options->delim == '\0' ||
...
static CsvStatus CsvWriterFlushBuf(CsvWriter *w)
{
    if (w->bufPos == 0)
        return CSV_STATUS_OK;

    if (fwrite(w->buf, 1, w->bufPos, w->stream) != w->bufPos)
        return CSV_STATUS_IO_ERROR;
...
static CsvStatus CsvWriterAppend(CsvWriter *w, const char *data, size_t len)
{
    while (len > 0)
    {
        size_t avail = w->bufCap - w->bufPos;
        if (avail == 0)
        {
            CsvStatus s = CsvWriterFlushBuf(w);
            if (s != CSV_STATUS_OK)
                return s;
            avail = w->bufCap;
        }
        ...
        memcpy(w->buf + w->bufPos, data, chunk);
        w->bufPos += chunk;
        data += chunk;
        len -= chunk;
    }
...
static CsvStatus CsvWriterAppendChar(CsvWriter *w, char ch)
{
    if (w->bufPos >= w->bufCap)
    {
        CsvStatus s = CsvWriterFlushBuf(w);
        if (s != CSV_STATUS_OK)
            return s;
...
static int CsvFieldNeedsQuoting(const char *data, size_t len,
                                char delim, char quote)
{
    for (size_t i = 0; i < len; i++)
    {
        char ch = data[i];
        if (ch == delim || ch == quote || ch == '\n' || ch == '\r')
            return 1;
    }
    return 0;
}

static CsvStatus CsvWriterInitInternal(CsvWriter **out_writer,
                                       FILE *stream,
                                       int ownsStream,
                                       const CsvOptions *options)
{
    CsvWriter *w = calloc(1, sizeof(CsvWriter));
    if (!w)
    {
        if (ownsStream)
            fclose(stream);
    ...
    w->buf = malloc(CSV_WRITER_BUF_SIZE);
    if (!w->buf)
    {
        free(w);
        if (ownsStream)
    ...
    w->bufCap = CSV_WRITER_BUF_SIZE;
    w->stream = stream;
    w->ownsStream = ownsStream;
    w->delim = options->delim;
    w->quote = options->quote;
...
CsvStatus csv_writer_open(CsvWriter **out_writer, const char *filename)
{
    ...
    return csv_writer_open_with_options(out_writer, filename, &options);
}

CsvStatus csv_writer_open_with_options(CsvWriter **out_writer,
                                       const char *filename,
                                       const CsvOptions *options)
{
    FILE *fp;

    if (!out_writer || !filename || !options)
        return CSV_STATUS_INVALID_ARGUMENT;

    *out_writer = NULL;

    CsvStatus vs = CsvWriterValidateOptions(options);
    if (vs != CSV_STATUS_OK)
        return vs;
    ...
    return CsvWriterInitInternal(out_writer, fp, 1, options);
}
...
CsvStatus csv_writer_open_stream(CsvWriter **out_writer, FILE *stream)
{
    ...
    return csv_writer_open_stream_with_options(out_writer, stream, &options);
}

CsvStatus csv_writer_open_stream_with_options(CsvWriter **out_writer,
                                              FILE *stream,
                                              const CsvOptions *options)
{
    if (!out_writer || !stream || !options)
        return CSV_STATUS_INVALID_ARGUMENT;
    ...
    *out_writer = NULL;
    ...
    return CsvWriterInitInternal(out_writer, stream, 0, options);
}
...
CsvStatus csv_writer_field(CsvWriter *writer, const char *data, size_t len)
{
    ...
    /* prepend delimiter between fields */
    if (writer->colIndex > 0)
    {
        s = CsvWriterAppendChar(writer, writer->delim);
        if (s != CSV_STATUS_OK)
            return s;
    }
    ...
    if (CsvFieldNeedsQuoting(data, len, writer->delim, writer->quote))
    {
        /* quoted output: quote char, escaped data, quote char */
        s = CsvWriterAppendChar(writer, writer->quote);
        if (s != CSV_STATUS_OK)
            return s;
        ...
        for (size_t i = 0; i < len; i++)
        {
            if (data[i] == writer->quote)
            {
                /* flush up to and including the quote */
                s = CsvWriterAppend(writer, start, (size_t)(data + i + 1 - start));
                if (s != CSV_STATUS_OK)
                    return s;
                /* write the doubled quote */
                s = CsvWriterAppendChar(writer, writer->quote);
                if (s != CSV_STATUS_OK)
                    return s;
                start = data + i + 1;
        ...
        if (start < data + len)
        {
            s = CsvWriterAppend(writer, start, (size_t)(data + len - start));
            if (s != CSV_STATUS_OK)
                return s;
        }
    ...
    {
        s = CsvWriterAppend(writer, data, len);
        if (s != CSV_STATUS_OK)
            return s;
    }
...
CsvStatus csv_writer_field_str(CsvWriter *writer, const char *str)
{
    ...
    return csv_writer_field(writer, str, strlen(str));
}
...
CsvStatus csv_writer_flush(CsvWriter *writer)
{
    ...
    return CsvWriterFlushBuf(writer);
}
...
void csv_writer_close(CsvWriter *writer)
{
    ...
    (void)CsvWriterFlushBuf(writer);

    if (writer->ownsStream)
